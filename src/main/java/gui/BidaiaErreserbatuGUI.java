...
public class BidaiaErreserbatuGUI extends JFrame {
	...
	private final JButton Menua = new JButton(ResourceBundle.getBundle("Etiquetas").getString("Menua")); //$NON-NLS-1$ //$NON-NLS-2$

	public BidaiaErreserbatuGUI(Traveler t)
	{
				...
				else if (propertychangeevent.getPropertyName().equals("calendar"))
				{
					...
					try {
						...
						for (domain.Ride ride:rides){
							...
							row.add(facade.LibreDaudenEserlekuKopurua(ride.getRideNumber()));        //Libre dauden eserlekuak
					...
					tableRides.getColumnModel().getColumn(1).setPreferredWidth(100);
		...
		JButton btnNewButton = new JButton(ResourceBundle.getBundle("Etiquetas").getString("Erreserbatu")); //$NON-NLS-1$ //$NON-NLS-2$
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				
				
				int errenkada=tableRides.getSelectedRow();
				if (errenkada != -1) { 
					
					Object value = tableRides.getValueAt(errenkada, 0);
					List<domain.Ride> rides=facade.getRides((String)jComboBoxOrigin.getSelectedItem(),(String)jComboBoxDestination.getSelectedItem(),UtilDate.trim(jCalendar1.getDate()));
					Ride bidaia=rides.get(errenkada);
				...
				else System.out.println("Ez da eserleku librerik geratzen");
				}
		...
		btnNewButton.setBounds(182, 417, 209, 27);
		getContentPane().add(btnNewButton);
		
		JButton DriverrarenProfilaIkusibtn = new JButton(ResourceBundle.getBundle("Etiquetas").getString("BidaiaErreserbatuGUI.DriverrarenProfila")); //$NON-NLS-1$ //$NON-NLS-2$
		DriverrarenProfilaIkusibtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int errenkada=tableRides.getSelectedRow();
				if (errenkada != -1) { 
					
					Object gidariaObj = tableRides.getValueAt(errenkada, 0);
					String gidariIzena = gidariaObj.toString();
					User ikusi = facade.getUser(gidariIzena);
					
					System.out.println(ikusi.getName() + "aaaaa");
					ArrayList<Balorazioa> balorazioak=facade.balorazioakKargatu(ikusi);
					for(Balorazioa b:balorazioak) {
						System.out.println(b.getNota());
					}
					
					
					jButton2_actionPerformed(e);
					ProfilaIkusiGUI Profila =new ProfilaIkusiGUI(t,ikusi);
					Profila.setVisible(true);
					
					
				}
				else {
					System.out.println("aukeratu bidaia bat");
				}
				
			}
		});
		DriverrarenProfilaIkusibtn.setBounds(6, 292, 148, 25);
		getContentPane().add(DriverrarenProfilaIkusibtn);
		Menua.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				jButton2_actionPerformed(arg0);
				TravelerMenuaGUI menua=new TravelerMenuaGUI(t);
				menua.setVisible(true);
			}
		});
		Menua.setBounds(12, 5, 105, 27);
